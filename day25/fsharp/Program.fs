...
let toNumber = function | '2' -> 2m | '1' -> 1m | '0' -> 0m | '-' -> -1m | '=' -> -2m | _ -> failwith "Unknown snafu digit"

let getDecimals (i: float) = i - Math.Truncate i
...
type Snafu = { underlyingValue: decimal }
...
let sum = File.ReadAllLines("input") |> Seq.map snafu |> Seq.reduce (+)
printfn $"{sum}"