...
let rec parse (reader: System.IO.StreamReader) (currentDir: DirectoryInfo) =
    ...
    match line with
    | Cd dir when dir = "/"     -> traverseToRoot currentDir |> parse reader
    | Cd dir when dir = ".."    -> parse reader currentDir.parent.Value
    | Cd dir                    -> getChildDirectory currentDir dir |> parse reader
    | Ls                        -> parse reader currentDir
    | Dir name                  -> currentDir.children.Add( Directory( { name = name ; parent = Some(currentDir) ; children = List<Item>() }))
                                   parse reader currentDir
    | File (size, name)         -> currentDir.children.Add(File({ name = name; size = size }))
                                   parse reader currentDir
    | EOF                       -> Directory(traverseToRoot currentDir)

let getDirectorySizes tree =
    let rec accumulateDirectories (directories: List<int>) acc =
        function
        | Item.File fileInfo        -> acc + fileInfo.size
        | Directory directoryInfo   -> let sizeOfChildren = directoryInfo.children |> Seq.fold (accumulateDirectories directories) 0
...
directories
    |> Seq.filter (fun x -> x >= neededSpace)
    |> Seq.min
    |> printfn "%i"
